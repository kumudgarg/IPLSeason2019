...
public class BatsmanAdapterTest {
    ...
    public void givenIPLMOstRunsCSVFile_ShouldReturnCorrectRecords() {
        try {
            IPLAdapter iplAdapter = new BatsmanAdapter();
            recordDAOMap = iplAdapter.loadIPLData(IPLEntity.BATING, IPL_MOST_RUNS_CSV_FILE_PATH);
            Assert.assertEquals(100, recordDAOMap.size());
    ...
    public void givenWrongIPLMOstRunsCSVFile_ShouldReturnCustomExceptionType() {
        try {
            IPLAdapter iplAdapter = new BatsmanAdapter();
            recordDAOMap = iplAdapter.loadIPLData(IPLEntity.BATING, IPL_Wrong_CSV_FILE_PATH);
        } catch (IPLCSVException e) {
    ...
    public void givenWrongDelimiterIPLMOstRunsCSVFile_ShouldReturnCustomExceptionType() {
        try {
            IPLAdapter iplAdapter = new BatsmanAdapter();
            recordDAOMap = iplAdapter.loadIPLData(IPLEntity.BATING, Wrong_Delimiter_CSV_FILE);
        } catch (IPLCSVException e) {
            Assert.assertEquals(IPLCSVException.ExceptionType.IPL_FILE_INTERNAL_ISSUES, e.type);
        }
    }
    ...
    public void givenMissingHeaderIPLMOstRunsCSVFile_ShouldReturnCustomExceptionType() {
        try {
            IPLAdapter iplAdapter = new BatsmanAdapter();
            recordDAOMap = iplAdapter.loadIPLData(IPLEntity.BATING, Missing_Header_CSV_File);
        } catch (IPLCSVException e) {
            Assert.assertEquals(IPLCSVException.ExceptionType.IPL_FILE_INTERNAL_ISSUES, e.type);
        }
    }
    ...
    public void givenNonExistingIPLMOstRunsCSVFile_ShouldReturnCustomExceptionType() {
        try {
            IPLAdapter iplAdapter = new BatsmanAdapter();
            recordDAOMap = iplAdapter.loadIPLData(IPLEntity.BATING, NON_Existing_IPL_CSV_File);
        } catch (IPLCSVException e) {
